...
public class MinesweeperGame {
    ...
    private static final Integer BOARD_ROW_SIZE = 8;
    private static final Integer BOARD_COL_SIZE = 10;
    ...
    private static final Integer LAND_MINE_COUNT = 10;
    ...
    private static void actOnCell(String cellInput, String userActionInput) {
        int selectedColIdx = getSelectedColIndex(cellInput);
        int selectedRow = getSelectedRowIndex(cellInput);

        if (doesUserChooseToPlantFlag(userActionInput)) {
            BOARD[selectedRow][selectedColIdx] = FLAG_SIGN;
            checkIfGameIsOver();
            return;
        }

        if (doesUserChooseToOpenCell(userActionInput)) {
            if (isLandMineCell(selectedRow, selectedColIdx)) {
                BOARD[selectedRow][selectedColIdx] = LAND_MINE_SIGN;
                changeGameStatusToLose();
                return;
            }

            open(selectedRow, selectedColIdx);
            checkIfGameIsOver();
            return;
        }
    ...
    private static boolean isLandMineCell(int selectedRow, int selectedColIdx) {
        return LAND_MINES[selectedRow][selectedColIdx];
    }
